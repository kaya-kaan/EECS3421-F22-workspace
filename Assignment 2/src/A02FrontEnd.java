...
public class A02FrontEnd extends JFrame implements ItemListener, ActionListener{
    ...
    JButton submitQuery; 
    int type = 0; // Determines which type of Event is being inserted. 
    A02MiddleTier db = new A02MiddleTier(); // Accesses the functions from the A02MiddleTier class.
    ...
    public void actionPerformed(ActionEvent e) {
        if (type == 0 || evNameT.getText() == null || jourNameT.getText() == null || evDateT == null) { // Invalid input check.
            queryOutput.setText("Please select an event type and make sure all text boxes are filled."); 
        }
        else { 
            if (type == 1) { // Insert eventConference
                try {
                    db.createEvent(evNameT.getText()); 
                    db.createEventConference(evDateT.getText()); 
                    queryOutput.setText("Updated Event Table:\n" + db.outputEvent() + "Updated EventConference Table:\n" + db.outputEventConference());
                } catch (SQLException e1) {

                    e1.printStackTrace();
                }
            }
            else if (type == 2) { // Insert eventJournal
                try {
                    db.createEvent(evNameT.getText()); 
                    db.createEventJournal(jourNameT.getText()); 
                    queryOutput.setText("Updated Event Table:\n" + db.outputEvent() + "Updated EventJournal Table:\n" + db.outputEventJournal());
                } catch (SQLException e1) {
                ...
                try { // Insert eventBook
                    db.createEvent(evNameT.getText()); 
                    db.createEventBook(); 
                    queryOutput.setText("Updated Event Table:\n" + db.outputEvent() + "Updated EventBook Table:\n" + db.outputEventBook());
                } catch (SQLException e1) {
